  ...
  const validateNationalId = (value: string) => {
    ...
    if (dateOfBirth) {
      ...
      if (!digits.startsWith(expectedPrefix)) {
        return t("validation.nationalId.birthYearMismatch").replace("{year}", birthYear)
      }
    }
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
          ...
          {errors.dateOfBirth && (
            <div className="flex items-center gap-1 mt-1">
              <AlertCircle className="w-4 h-4 text-red-500" />
              <span className="text-sm text-red-600">{errors.dateOfBirth.message}</span>
            </div>
          )}
        </div>
        ...
        <div >
          ...
          <input
            ...
            placeholder={`784-${getBirthYearFromDate(dateOfBirth) || "YYYY"}-XXXXXXX-X`}
            maxLength={17}
            className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent ${
            ...
            dir={isRTL ? "rtl" : "ltr"}
          />
          <div className="text-xs text-gray-500 mt-1">{t("validation.nationalId.formatHelp")}</div>
          {errors.nationalId && (
            <div className="flex items-center gap-1 mt-1">
              <AlertCircle className="w-4 h-4 text-red-500" />
              <span className="text-sm text-red-600">{errors.nationalId.message}</span>
            </div>
          )}
        </div>
